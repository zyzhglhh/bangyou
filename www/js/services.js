...
.factory('Data', ['$resource', '$q', 'CONFIG', '$interval', function ($resource, $q, CONFIG, $interval) {
  ...
  var Consumption = function () {
    return $resource(CONFIG.baseUrl + ':path/:route', {
      ...
      hljorder: {method:'POST', params:{route: 'hljorder'}, timeout: 10000},
      ghorder: {method:'POST', params:{route: 'ghorder'}, timeout: 10000},
...
.factory('Consumption', ['Storage', 'Data', 'Token', '$state', '$q', 'jwtHelper', function (Storage, Data, Token, $state, $q, jwtHelper) {
  return {
    ...
    ghOrder: function(query, options, fields) {
      var deferred = $q.defer();
      Data.Consumption.ghorder({query:query, options: options, fields: fields}, function(data, headers) {
        deferred.resolve(data);
      }, function(err) {
        deferred.reject(err);
      });
    ...
    hljOrder: function(query, options, fields) {
      var deferred = $q.defer();
      Data.Consumption.hljorder({query:query, options: options, fields: fields}, function(data, headers) {
        deferred.resolve(data);
      }, function(err) {
        deferred.reject(err);
      });
